...
module.exports = class extends DeliveryRepository {
    ...
    async start(id, status, role, userId) {
        ...
        let totalRoutes = mongoResult.routes.length;
        let totalRoutesFinished = 0;
        let result = null;

        for(let route of mongoResult.routes){
            if(route.status === "Finalizada" || route.status === "Cancelada"){
                totalRoutesFinished++;
                if(totalRoutesFinished == totalRoutes){
                    response.message = 'Esta entrega ya se encuentra finalizada';
                    response.error = true;
                    await MongooseDelivery.updateOne({ _id: objectId(id) }, { status: 'Finalizada' });
                    return response;
                }
            ...
            if(route.status === "En proceso"){
                response.message = 'Una entrega se encuentra en proceso, no puede iniciar otra';
                response.error = true;
                return response;
            }
            if(route.status === "Pendiente"){
                result = await MongooseDelivery.updateOne({ _id: objectId(id), 'routes._id': objectId(route._id) }, { $set: { 'routes.$.status': status } });
                break;
            }
        }
    ...
    async end(id, annexes, role, userId) {
        let response = {
            data: {},
            message: null,
            error: false
        ...
        if(mongoResult.status === "Pendiente"){
            response.message = 'La entrega no ha sido iniciada';
            response.error = true;
            return response;
        }
        ...
        let rts = mongoResult.routes.map((route) => {
            if(route.status === "Finalizada" || route.status === "Cancelada"){
                totalRoutesFinished++;
            }
            if(route.status === "En proceso"){
        ...
        if(isInProcess === false){
            response.message = 'No hay ninguna ruta en proceso';
            response.error = true;
            return response;
        }

        if(totalRoutesFinished == totalRoutes){
            await MongooseDelivery.updateOne({ _id: objectId(id) }, {$set: {status: 'Finalizada'}, routes: rts });
            response.message = 'Todas las rutas ya se encuentran finalizadas con éxito';
        }else{
            await MongooseDelivery.updateOne({ _id: objectId(id) }, { routes: rts });
            response.message = 'Entrega a la colonia finalizada correctamente';
        }
    ...
    async cancel(id, annexes, role, userId) {
        let response = {
            data: {},
            message: null,
            error: false
        ...
        if (role === 'Administrador') {
            response.message = 'El usuario no tiene permisos para cancelar una entrega';
            response.error = true;
            return response;
        }
        ...
        let result = await MongooseDelivery.updateOne({ _id: objectId(id) }, {  generalAnnexes: annexes });

        mongoResult = await MongooseDelivery.findOne({ _id: objectId(id) });

        response.data['delivery'] = new Delivery(mongoResult.id, mongoResult.name, mongoResult.routes, mongoResult.user, mongoResult.date, mongoResult.annexes, mongoResult.status);
