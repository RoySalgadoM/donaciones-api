...
module.exports = class extends DeliveryRepository {
    ...
    async end(id, annexes, role, userId) {
        ...
        if(totalRoutesFinished == totalRoutes){
            ...
            response.message = 'Todas las entregas ya se encuentran finalizadas con éxito';
        }else{
    ...
    async cancel(id, annexes, role, userId) {
        ...
        if (role === 'Administrador') {
            response.message = 'El usuario no tiene permisos para cancelar la entrega a una colonia';
            response.error = true;
            return response;
        }
