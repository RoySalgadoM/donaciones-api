...
module.exports = class extends UserRepository {
  ...
  async changeStatus(id, status) {
    ...
    if (user.status === status) {
      response.message = `El usuario ya se encuentra en el estado  ${status ? 'Activo' : 'Inactivo'}, actualiza la página para ver los cambios.`;
      response.error = true;
      return response;
    }
  ...
  async get(page, rowsPerPage, order, orderBy, filter) {
    let response = {
      data: {},
      message: null,
      error: false
    ...
    if (filter) {
      filterQuery = {
        $or: [
          ...
          { secondSurname: { $regex: filter, $options: 'i' } },
    ...
    const users = await MongooseUser.find(filterQuery).sort(sortQuery).skip(skip).limit(limit);
  ...
  async getEmployees(page, rowsPerPage, order, orderBy, filter) {
    let response = {
      data: {},
      message: null,
      error: false
    ...
    response.message = 'Empleados recuperados correctamente';
    response.data['employees'] = users.map(user => new User(user.id, user.name, user.lastname, user.secondSurname, user.email, null, user.role, null, user.phone, user.status));
    response.data['total'] = count;
    return response;
  }
