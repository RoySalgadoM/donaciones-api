...
const Cancel = require('../../application/use_cases/pickup/Cancel');
const Update = require('../../application/use_cases/pickup/Update');


const objectId = require('mongoose').Types.ObjectId;
const MongoosePickup = require('../../infrastructure/orm/mongoose/schemas/Pickup');
...
module.exports = {
    ...
    async update(req, h) {
        try {
            ...
            const serviceLocator = req.server.app.serviceLocator;
            const result = await Update({ id, name, chain, products, user, date, role, userId }, serviceLocator);

            if (result.error) {
                if (result.message === "El usuario no tiene permisos para editar una recolección") {
                    return Boom.forbidden(result.message);
                }
                return Boom.badRequest(result.message);
            }
    ...
    async getPendients(req, h) {
        try {
            ...
            let totalPendings = await MongoosePickup.count({ "user._id": objectId(userId), status: { $in: ['Pendiente'] } });

            let response = {
                statusCode: 200,
                message: "Se ha obtenido el total de recolecciones pendientes",
                data: {
                    count: totalPendings
                }
            }
